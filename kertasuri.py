...
def watermark():
    print("developer kerta dc kerta123 :)")

def get_number():
    global clear
    while True:
        try:
            number = int(input(f"""Telefon numarasını yazın örnek 5550005555 bilmiyorsan sg (dev kerta)\n[?] : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"Numara Yanlış yazmayı ogren once amına goyım.")
        except:
            clear()
